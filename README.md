...
- 🟢 **Green rows** — Status code matches baseline (expected behavior)
- 🟡 **Yellow/highlighted rows** — Anomalies detected (investigate these!)

**Dynamic Sorting:** Click any column header to instantly sort results by **Status Code**, **Word Count**, **Length**, or **RTT**. This is critical during large-scale testing — instantly isolate the `200 OK` responses among hundreds of `403`s, find the smallest response that might indicate a different code path, or spot the slowest response that could signal server-side processing. No need to scroll through hundreds of rows; **one click reveals your bypass**.
...
| Feature | How |
|---------|-----|
| **Send to Repeater** | Right-click any row → *Send to Repeater* |
...
| 7 | Double Slash | `Advanced Encoders` | `//admin` | **Nginx/OpenResty** proxy path confusion — targets misconfigurations in `location` block matching |
